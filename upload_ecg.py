...
# Specify the file paths for the ECG data files
file_paths = [
    "D:\\Projects\\ECGAnalysisDashbaord\\mit-bih-arrhythmia-database-1.0.0\\100",
    "D:\\Projects\\ECGAnalysisDashbaord\\mit-bih-arrhythmia-database-1.0.0\\101"
]

# Loop through the specified file paths
for path in file_paths:
    ...
    if os.path.exists(dat_file) and os.path.exists(hea_file):
        st.write(f"Loading files: {hea_file}, {dat_file}")
        ...
        # Optionally, display the first 1000 points of the first signal
        st.write("First 1000 samples of the ECG signal:")
        st.line_chart(record.p_signal[:1000, 0])
    else:
        st.error(f"Files not found: {hea_file} or {dat_file}")
